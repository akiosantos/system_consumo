...
SYSTEM_DIR = r"_caminho_da_pasta"
BASE_DIR = os.path.join(SYSTEM_DIR, "backend")
FRONT_DIR = os.path.join(SYSTEM_DIR, "frontend")

PLANILHA_ENEL = r"_caminho_da_past"
ABA_ENEL = "ENEL 2026"

PLANILHA_SABESP = r"_caminho_da_past\Sabesp 2026.xlsx"
ABA_SABESP = "SABESP 2026"

PDF_SABESP_COM_CODIGO = os.path.join(BASE_DIR, "sabesp_com_codigo.pdf")

# ===== SABESP =====
EMAIL_SABESP = os.getenv("EMAIL_SABESP", "seu.email.sp.gov.br")
SENHA_SABESP = os.getenv("SENHA_SABESP", "suasenha1234")
REMETENTES_SABESP = [
    "femail-remetentep@sabesp.com.br",
    "e-mailremetente2@barueri.sp.gov.br"
]
...
# ===== ENEL =====
EMAIL_ENEL = os.getenv("EMAIL_ENEL", "seu-email.sp.gov.br")
SENHA_ENEL = os.getenv("SENHA_ENEL", "senha13234")
REMETENTE_ENEL = [
    "enel.com",
    "seuemail@gov.br"
]
...
def extrair_instalacao(texto):
    """
    VERSÃO CORRIGIDA - Maio 2026
    
    Problema: As páginas 397-398 têm o padrão "Número UC" + MTE
    Solução: Detectar este padrão PRIMEIRO nas estratégias
    """
    
    # ESTRATÉGIA 0 (NOVA): Padrão "Número UC" → MTE****
    # Detecta páginas 397-398
    m_numero_uc = re.search(
        r'Número\s+UC\s*\n\s*(MTE\d+)',
        texto,
        re.IGNORECASE
    )
    if m_numero_uc:
        return m_numero_uc.group(1).upper()
    
    # ESTRATÉGIA 1: Procurar por MTE
    m_mte = re.search(r"\b(MTE[A-Z0-9]{5,15})\b", texto, re.IGNORECASE)
    if m_mte:
        return m_mte.group(1).upper()
    
    # ESTRATÉGIA 2: Procurar por "UC TOTAL A PAGAR" + número 10-12 dígitos
    m_uc = re.search(
        r'UC\s*TOTAL\s*A\s*PAGAR\s*[^\d]*?(\d{10,12})\s*/?\s*(\d{6,9})?',
        texto,
        re.IGNORECASE | re.DOTALL
    )
    if m_uc:
        numero_uc = m_uc.group(1)
        if numero_uc.startswith('0') or len(numero_uc) == 10:
            return normalizar_instalacao(numero_uc)
    
    # ESTRATÉGIA 3: Número 10 dígitos começando com 0
    m_padrao_uc = re.search(
        r'(?:UC\s+)?(\d{10})\s*(?:/|[\s\n])',
        texto,
        re.IGNORECASE
    )
    if m_padrao_uc:
        numero = m_padrao_uc.group(1)
        if numero.startswith('0'):
            return normalizar_instalacao(numero)
    
    # ESTRATÉGIA 4: Último recurso - número genérico
    ...
    if numeros:
        ...
        numeros_unicos = [n for n, c in contador.items() if c == 1]
        if numeros_unicos:
            return normalizar_instalacao(numeros_unicos[0])
...
def extrair_referencia(texto, instalacao):
    ...
    Problema: Algumas páginas não extraem a referência (mês/ano)
    Causa: A busca era limitada a uma janela pequena perto da instalação
    Solução: Estratégias em cascata para encontrar a data de referência
    """
    
    if not instalacao:
        return ""
    
    # Normalizar instalação para busca
    instalacao_clean = instalacao.lstrip("0")
    
    # ===== ESTRATÉGIA 1: Buscar padrão "REFERÊNCIA: MM/YYYY" =====
    # Alguns documentos têm o padrão explícito
    m_ref_label = re.search(
        r'REFERÊNCIA\s*:\s*([0-9]{2})/([0-9]{4})',
        texto,
        re.IGNORECASE
    )
    if m_ref_label:
        mes = m_ref_label.group(1)
        ano = m_ref_label.group(2)
        # Validar: mês deve ser 01-12
        if 1 <= int(mes) <= 12:
            return f"{mes}/{ano}"
    
    # ===== ESTRATÉGIA 2: Buscar "MÊS/ANO" label =====
    # Procura pela label "MÊS/ANO" e depois pega a data ao lado
    m_mes_ano_label = re.search(
        r'MÊS\s*/\s*ANO\s+([0-9]{2})/([0-9]{4})',
        texto,
        re.IGNORECASE
    )
    if m_mes_ano_label:
        mes = m_mes_ano_label.group(1)
        ano = m_mes_ano_label.group(2)
        if 1 <= int(mes) <= 12:
            return f"{mes}/{ano}"
    
    # ===== ESTRATÉGIA 3: Data na mesma região que a instalação =====
    # Se encontrar a instalação, procura data até 500 chars à frente
    t = re.sub(r"\s+", " ", texto)
    instalacao_clean_lower = instalacao_clean.lower()
    
    pos = t.lower().find(instalacao_clean_lower)
    if pos != -1:
        # Pega 500 chars a partir da posição da instalação
        area = t[pos:pos+500]
        
        # Remove datas longas (DD/MM/YYYY) para não confundir com referência
        area_cleaned = re.sub(r"\b\d{2}/\d{2}/\d{4}\b", "", area)
        
        # Procura MM/YYYY (01-12)
        m_date = re.search(r"\b(0[1-9]|1[0-2])/([0-9]{4})\b", area_cleaned)
        if m_date:
            return f"{m_date.group(1)}/{m_date.group(2)}"
    
    # ===== ESTRATÉGIA 4: Buscar todas as datas MM/YYYY no documento =====
    # Remove datas longas (que são datas de vencimento, emissão, etc)
    texto_cleaned = re.sub(r"\b\d{2}/\d{2}/\d{4}\b", "", texto)
    
    # Procura todas as datas MM/YYYY válidas
    datas = re.findall(r"\b(0[1-9]|1[0-2])/([0-9]{4})\b", texto_cleaned)
    
    if datas:
        # Trata-se de um documento de faturamento, a primeira data é geralmente a referência
        # Mas às vezes há múltiplas. Pega a primeira que aparece após descartar datas longas
        mes, ano = datas[0]
        return f"{mes}/{ano}"
    
    # ===== ESTRATÉGIA 5: Fallback - procurar MM/ANO com qualquer MM =====
    # Último recurso: procura qualquer padrão MM/YYYY mesmo que MM seja 00 ou 13+
    # (teoricamente inválido, mas melhor retornar algo do que nada)
    m_any_date = re.search(r"(\d{2})/(\d{4})", texto)
    if m_any_date:
        mes = m_any_date.group(1)
        ano = m_any_date.group(2)
        # Só retorna se parecer válido (01-12)
        if 1 <= int(mes) <= 12:
            return f"{mes}/{ano}"
...
if __name__ == "__main__":
    ...
    # ===== TESTE 1: Página 397 =====
    print("\n✅ TESTE 1: Página 397 (Instalação)")
    texto_397 = """
    Número UC
    MTE0016908 / 300000003653 R$4.900,65
    """
    resultado = extrair_instalacao(texto_397)
    esperado = "MTE0016908"
    ...
    # ===== TESTE 2: Página 398 =====
    print("\n✅ TESTE 2: Página 398 (Instalação)")
    texto_398 = """
    Número UC
    MTE0013918 / 300000003424 R$11.360,59
    """
    resultado = extrair_instalacao(texto_398)
    esperado = "MTE0013918"
    status = "✅ PASSOU" if resultado == esperado else "❌ FALHOU"
    print(f"  Esperado: {esperado}")
    print(f"  Obtido:   {resultado}")
    ...
    # ===== TESTE 3: Página 1 (normal) =====
    print("\n✅ TESTE 3: Página 1 (Instalação normal)")
    texto_1 = """
    UC
    MTE0014625 / 300000003130
    """
    resultado = extrair_instalacao(texto_1)
    esperado = "MTE0014625"
    status = "✅ PASSOU" if resultado == esperado else "❌ FALHOU"
    print(f"  Esperado: {esperado}")
    print(f"  Obtido:   {resultado}")
    ...
    # ===== TESTE 4: Página 2 (Referência) =====
    print("\n✅ TESTE 4: Página 2 (Referência com REFERÊNCIA: label)")
    texto_ref_label = """
    NOTA FISCAL Nº 052610974 - Série 002 / DATA DE EMISSÃO: 03/04/2026 
    REFERÊNCIA: 04/2026
    DATA DE VENCIMENTO: 25/05/2026
    """
    resultado = extrair_referencia(texto_ref_label, "3554265")
    esperado = "04/2026"
    status = "✅ PASSOU" if resultado == esperado else "❌ FALHOU"
    print(f"  Esperado: {esperado}")
    print(f"  Obtido:   {resultado}")
    ...
    # ===== TESTE 5: Página normal (Referência) =====
    print("\n✅ TESTE 5: Página normal (Referência com MÊS/ANO label)")
    texto_mes_ano = """
    MTE0014625 / 300000003130
    MÊS/ANO CONSUMO FATURADO
    04/2026 150
    DATA DE VENCIMENTO: 25/05/2026
    """
    resultado = extrair_referencia(texto_mes_ano, "MTE0014625")
    esperado = "04/2026"
    status = "✅ PASSOU" if resultado == esperado else "❌ FALHOU"
    print(f"  Esperado: {esperado}")
    print(f"  Obtido:   {resultado}")
    ...
    # ===== TESTE 6: Página com múltiplas datas =====
    print("\n✅ TESTE 6: Múltiplas datas (pega primeira válida)")
    texto_multiplas = """
    0039544265 / 300009708919
    LEITURA ANTERIOR: 03/03/2026
    LEITURA ATUAL: 02/04/2026
    REFERÊNCIA: 04/2026
    VENCIMENTO: 25/05/2026
    """
    resultado = extrair_referencia(texto_multiplas, "39544265")
    esperado = "04/2026"
    status = "✅ PASSOU" if resultado == esperado else "❌ FALHOU"
    print(f"  Esperado: {esperado}")
    print(f"  Obtido:   {resultado}")
    ...
    # ===== TESTE 7: Página sem label (fallback) =====
    print("\n✅ TESTE 7: Sem label (estratégia fallback)")
    texto_fallback = """
    MUNICIPIO DE BARUERI
    R JANDIRA GUERRA 60 CS 2
    Monofásico
    Nº 510021953990
    30
    04/2026
    VENCIMENTO: 25/05/2026
    """
    resultado = extrair_referencia(texto_fallback, "3554265")
    esperado = "04/2026"
    status = "✅ PASSOU" if resultado == esperado else "❌ FALHOU"
    print(f"  Esperado: {esperado}")
    print(f"  Obtido:   {resultado}")
...
def extrair_total(texto):
    texto = texto.lower()  # Normaliza aqui internamente
...
def escrever_codigo_e_ordenar(pdf_entrada, pdf_saida, mapa):
    ...
    for page in reader.pages:
        texto = page.extract_text() or ""
        # ✅ CORRIGIDO: Sem normalizar() para preservar maiúsculas do MTE
        inst = extrair_instalacao(texto)
